...
type ServicioFormDefaults = Pick<NewServicio, 'id' | 'activo' | 'incidentes' | 'proveedorDelQueDependes'>;
...
type ServicioFormGroupContent = {
  ...
  proveedorDelQueDependes: FormControl<IServicio['proveedorDelQueDependes']>;
};
...
export class ServicioFormService {
  createServicioFormGroup(servicio?: ServicioFormGroupInput): ServicioFormGroup {
    ...
    return new FormGroup<ServicioFormGroupContent>({
      ...
      proveedorDelQueDependes: new FormControl(servicioRawValue.proveedorDelQueDependes ?? []),
    });
  }
