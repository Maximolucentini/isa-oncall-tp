...
import turnoDeProveedorResolve from './turno-de-proveedor-routing-resolve.service';

describe('TurnoDeProveedor routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ITurnoDeProveedor returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          turnoDeProveedorResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(turnoDeProveedorResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
