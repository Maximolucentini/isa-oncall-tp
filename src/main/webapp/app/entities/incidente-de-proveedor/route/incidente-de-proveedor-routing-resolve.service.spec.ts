...
import incidenteDeProveedorResolve from './incidente-de-proveedor-routing-resolve.service';

describe('IncidenteDeProveedor routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IIncidenteDeProveedor returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          incidenteDeProveedorResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(incidenteDeProveedorResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
