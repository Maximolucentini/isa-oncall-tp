...
import { sampleWithRequiredData } from '../servicio-de-proveedor.test-samples';

import { ServicioDeProveedor } from './servicio-de-proveedor';

vitest.useFakeTimers();

describe('ServicioDeProveedor Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ServicioDeProveedor;
  let fixture: ComponentFixture<ServicioDeProveedor>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ServicioDeProveedor);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6402 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.servicioDeProveedors()[0]).toEqual(expect.objectContaining({ id: 6402 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6402 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.servicioDeProveedors()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6402 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to servicioDeProveedorService', () => {
      ...
      vitest.spyOn(service, 'getServicioDeProveedorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getServicioDeProveedorIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.servicioDeProveedorsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.servicioDeProveedorsParams()).toMatchObject(
