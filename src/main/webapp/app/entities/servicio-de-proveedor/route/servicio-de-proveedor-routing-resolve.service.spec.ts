...
import servicioDeProveedorResolve from './servicio-de-proveedor-routing-resolve.service';

describe('ServicioDeProveedor routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IServicioDeProveedor returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          servicioDeProveedorResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(servicioDeProveedorResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
