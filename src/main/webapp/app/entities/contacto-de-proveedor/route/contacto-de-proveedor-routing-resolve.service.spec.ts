...
import contactoDeProveedorResolve from './contacto-de-proveedor-routing-resolve.service';

describe('ContactoDeProveedor routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IContactoDeProveedor returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          contactoDeProveedorResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(contactoDeProveedorResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
