...
public class Servicio implements Serializable {
    ...
    @ManyToMany(fetch = FetchType.LAZY, mappedBy = "servicioInternos")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "proveedor", "servicioInternos", "tickets" }, allowSetters = true)
    private Set<ServicioDeProveedor> proveedorDelQueDependes = new HashSet<>();
    ...
    public Set<ServicioDeProveedor> getProveedorDelQueDependes() {
        return this.proveedorDelQueDependes;
    }

    public void setProveedorDelQueDependes(Set<ServicioDeProveedor> servicioDeProveedors) {
        if (this.proveedorDelQueDependes != null) {
            this.proveedorDelQueDependes.forEach(i -> i.removeServicioInterno(this));
        }
        if (servicioDeProveedors != null) {
            servicioDeProveedors.forEach(i -> i.addServicioInterno(this));
        }
        this.proveedorDelQueDependes = servicioDeProveedors;
    }

    public Servicio proveedorDelQueDependes(Set<ServicioDeProveedor> servicioDeProveedors) {
        this.setProveedorDelQueDependes(servicioDeProveedors);
        return this;
    }

    public Servicio addProveedorDelQueDepende(ServicioDeProveedor servicioDeProveedor) {
        this.proveedorDelQueDependes.add(servicioDeProveedor);
        servicioDeProveedor.getServicioInternos().add(this);
        return this;
    }

    public Servicio removeProveedorDelQueDepende(ServicioDeProveedor servicioDeProveedor) {
        this.proveedorDelQueDependes.remove(servicioDeProveedor);
        servicioDeProveedor.getServicioInternos().remove(this);
        return this;
    }
