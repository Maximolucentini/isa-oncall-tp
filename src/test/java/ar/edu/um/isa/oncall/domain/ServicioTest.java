...
class ServicioTest {
    ...
    @Test
    void proveedorDelQueDependeTest() {
        Servicio servicio = getServicioRandomSampleGenerator();
        ServicioDeProveedor servicioDeProveedorBack = getServicioDeProveedorRandomSampleGenerator();

        servicio.addProveedorDelQueDepende(servicioDeProveedorBack);
        assertThat(servicio.getProveedorDelQueDependes()).containsOnly(servicioDeProveedorBack);
        assertThat(servicioDeProveedorBack.getServicioInternos()).containsOnly(servicio);

        servicio.removeProveedorDelQueDepende(servicioDeProveedorBack);
        assertThat(servicio.getProveedorDelQueDependes()).doesNotContain(servicioDeProveedorBack);
        assertThat(servicioDeProveedorBack.getServicioInternos()).doesNotContain(servicio);

        servicio.proveedorDelQueDependes(new HashSet<>(Set.of(servicioDeProveedorBack)));
        assertThat(servicio.getProveedorDelQueDependes()).containsOnly(servicioDeProveedorBack);
        assertThat(servicioDeProveedorBack.getServicioInternos()).containsOnly(servicio);

        servicio.setProveedorDelQueDependes(new HashSet<>());
        assertThat(servicio.getProveedorDelQueDependes()).doesNotContain(servicioDeProveedorBack);
        assertThat(servicioDeProveedorBack.getServicioInternos()).doesNotContain(servicio);
    }
}
