...
class ServicioResourceIT {
    ...
    @Test
    @Transactional
    void getAllServiciosByProveedorDelQueDependeIsEqualToSomething() throws Exception {
        ServicioDeProveedor proveedorDelQueDepende;
        if (TestUtil.findAll(em, ServicioDeProveedor.class).isEmpty()) {
            servicioRepository.saveAndFlush(servicio);
            proveedorDelQueDepende = ServicioDeProveedorResourceIT.createEntity(em);
        } else {
            proveedorDelQueDepende = TestUtil.findAll(em, ServicioDeProveedor.class).get(0);
        }
        em.persist(proveedorDelQueDepende);
        em.flush();
        servicio.addProveedorDelQueDepende(proveedorDelQueDepende);
        servicioRepository.saveAndFlush(servicio);
        Long proveedorDelQueDependeId = proveedorDelQueDepende.getId();
        // Get all the servicioList where proveedorDelQueDepende equals to proveedorDelQueDependeId
        defaultServicioShouldBeFound("proveedorDelQueDependeId.equals=" + proveedorDelQueDependeId);

        // Get all the servicioList where proveedorDelQueDepende equals to (proveedorDelQueDependeId + 1)
        defaultServicioShouldNotBeFound("proveedorDelQueDependeId.equals=" + (proveedorDelQueDependeId + 1));
    }
